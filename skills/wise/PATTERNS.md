...
Reference document for the **wise** skill. SKILL.md states the principles; this file provides concrete code-level examples.
...
// CORRECT (alternative): Atomic conditional update
affected = UPDATE records
    SET status = 'processing', started_at = now()
    WHERE id = ? AND status = 'pending'
if affected == 0:
    // Already claimed by another process — handle gracefully
```
...
// CORRECT: Persist error state outside the transaction
try:
    begin_transaction()
        do_work()
    end_transaction()
...
test_with_value(0)   // boundary — should this pass or fail?
...
When modifying code that has no existing tests:

```
// Step 1: Write a test that captures CURRENT behavior
def test_current_behavior():
    result = legacy_function(known_input)
    assert result == <whatever it actually returns>

// Step 2: Repeat for several representative inputs
// Step 3: NOW you have a safety net — make your changes
// Step 4: Any test that breaks tells you exactly what behavior changed
```
...
grep -rn "def method_name\|function method_name" src/
