# Attack On Hacker — Methodology

This is a **side-effect-free shared reference**. It is read by two callers:

- the `/attack-on-hacker` skill (explicit user invocation)
- the `/wise-flow` security gate, which cannot invoke that skill
  (`disable-model-invocation: true` means only the user can start it)

It grants nothing by itself: no `allowed-tools`, no preapproved commands. When
this file is read outside `/attack-on-hacker`, every command goes through the
normal permission prompt. The review task (target, diff range, context) comes
from the caller.

Every `references/<name>.md` mention below resolves against the
**attack-on-hacker skill directory** — repository path
`skills/attack-on-hacker/`, installed path `.claude/skills/attack-on-hacker/`
— never against the
caller's directory. So `references/quick-wins.md` means
`skills/attack-on-hacker/references/quick-wins.md` (installed:
`.claude/skills/attack-on-hacker/references/quick-wins.md`). A caller reading
this from wise-flow must load `diff-mode.md`, `quick-wins.md`,
`language-hints.md`, and `report-format.md` from there, or the quick-wins
sweep and the report contract silently drop out of the review.
...
- Keep the review read-only. Never run a repair/update mode such as
  `pip-audit --fix`, `cargo audit fix`, `detect-secrets scan --baseline`, or
  `safety scan --apply-fixes` without a separate explicit user authorization;
  those forms are intentionally not preapproved.
- `rg` and the scanners `gosec`, `trufflehog`, `gitleaks`, `checkov`, `tfsec`,
  and `bandit` are deliberately not preapproved either: a single flag turns
  each into a write or execute primitive (`rg --pre <cmd>`, scanner report
  paths), and a prefix rule cannot see flags. Expect one permission prompt per
  scanner run; for text search, use the built-in Grep tool instead of `rg`.
- Ranged or flagged `git diff` / `git log`, `bundle audit` subcommands, and
  flagged `yarn audit` forms are not preapproved either: `--output` /
  `--ext-diff` turn a diff into a write or an execute, `bundle audit update`
  mutates the advisory database, `yarn audit --mutex` writes a lock file or
  opens a network port, and expansion tricks such as `--out${GAP}put=/tmp/x`
  smuggle such flags past any string-level guard. Only the bare, flagless
  forms run without a prompt — expect a permission prompt for each ranged
  diff command.
...
If the review task indicates a diff or PR review, **read `references/diff-mode.md`
...
Use the built-in Grep tool, targeted file reads, and diffs to follow the shortest credible path through the code (`rg` is deliberately not preapproved — see Core Rules).
